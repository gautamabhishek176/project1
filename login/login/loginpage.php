<html>
<head>
<link rel="stylesheet" type="text/css" href="./login/loginpage1.css">
  ...
  <?php
	...
	$user_pass=mysqli_real_escape_string($mysql_connect,$user_pass);
	 ...
	 if(!empty($user_name)&&!empty($user_pass)){  // to check if they empty in any case
		$my_sql='SELECT `username` , `password`, `name`, `batch`, `branch` FROM `users` WHERE `username`= ?';
					 ...
					 if($no){
						 if($row=mysqli_fetch_assoc($res)){
							// echo $row['user_name_t'];
							 //echo $row['user_pass'];
							 //echo $row['teach_name'];
							 $password_check=password_verify($user_pass,$row['password']);
...
<body>
	...
	<form method="POST" action="<?php echo $current_page; ?>" onsubmit="return onsub()">
		   ...
		   <a href="http://localhost/project1/login/register/registration.php">Don't have an account?</a>
	  </form>	   
	 </div>
